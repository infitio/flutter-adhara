...
typedef Widget RouterWidgetBuilder(BuildContext context, RoutableURL matchingUrl, Widget widget);
...
class Router{
  ...
  static getAdharaRoute(RouteSettings routeSettings, RoutableURL url, RouterWidgetBuilder builder){
    ...
    for (URL url in urls) {
      ...
      if(url.module!=null){
        for(URL moduleURL in url.module.urls){
          ///If no match, then this URL is no good
          if (Router.getMatch(routeSettings.name, moduleURL.getPattern(url)) == null) {
            continue;
          }
          ...
          return Router.getAdharaRoute(routeSettings, routableModuleURL, builder);
        }
      }
