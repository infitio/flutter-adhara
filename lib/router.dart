...
Route getRoute(RouteSettings routeSettings, List<Map<String, dynamic>> namedPatternRoutes){
  ...
  for(Map<String, dynamic> config in namedPatternRoutes){
    ...
    List pathParamKeys = RegExp("{{([a-zA-Z\$_][a-zA-Z0-9\$_]*)}}")
      .allMatches(urlPattern).map((Match m) => m.group(1)).toList();

    ///Cleanup the url pattern and remove all mustache patterns, viz., {{asd092s1}}
    urlPattern = urlPattern.replaceAll(RegExp("\\{\\{[a-zA-Z\$_][a-zA-Z0-9\$_]*\\}\\}"), '');
